...
#include <fstream>
using namespace std;
//function coverting rgba to gray scale
//returns value varing from 0 to 7
...
int main(int argc, char* argv[])
{
	...
	fstream str;//stream for saving output
	...
	if (surface != NULL)//if surface is loaded properly
	{
		...
		//save the output to path.txt
		auto iter = path->find_last_of(".");
		path->erase(path->begin()+iter,path->end());
		*path += ".txt";
		str.open(path->c_str(), ios_base::trunc | ios_base::out | ios_base::in);
		str << *output;
		str.close();
	}
